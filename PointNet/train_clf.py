...
def parse_args():
    ...
    parser.add_argument('--batchsize', type=int, default=14, help='batch size in training')
    ...
    parser.add_argument('--model_name', default='pointnet', help='range of training rotation')
...
def main(args):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max',factor=0.5, verbose=True, min_lr=1e-7)
    ...
    for epoch in range(start_epoch,args.epoch):
        ...
        meter['train_acc'].append(train_acc)
        meter['test_acc'].append(acc)
    ...
    plt.plot(range(args.epoch), meter['train_acc'], label='Train Accuracy')
    plt.plot(range(args.epoch), meter['test_acc'], label='Test Accuracy')
    plt.grid(True)
    plt.title('Accuracy Curve')
    plt.legend()
    plt.savefig(str(experiment_dir / 'accuracy.png'))
